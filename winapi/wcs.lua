...
function mbs(ws, wl) --accept and convert a wcs or pwcs buffer to a lua string
	if ffi.istype(WCS_ctype, ws) or ffi.istype(PWCS_ctype, ws) then
		wl = wl or 1+wcslen(ws)
		local sz = checknz(WC2MB(CP_UTF8, WC, ws, wl, nil, 0, nil, nil))
		local buf = MBS_ctype(sz)
		checknz(WC2MB(CP_UTF8, WC, ws, wl, buf, sz, nil, nil))
