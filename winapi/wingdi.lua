...
function CreateDIBSection(hdc, bmi, usage, hSection, offset, bits)
	...
	local hbitmap = checkh(C.CreateDIBSection(hdc, bmi, flags(usage or DIB_RGB_COLORS), bits, hSection, offset or 0))
...
function StretchBlt(dst, x, y, w, h, src, x1, y1, cx1, cy1, rop)
	return checknz(C.StretchBlt(dst, x, y, w, h, src, x1, y1, cx1, cy1, flags(rop)))
end
