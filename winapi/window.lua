...
function ClientToScreen(hwnd, pt)
  if ffi.istype('POINT', pt) then
    return C.ClientToScreen(hwnd, pt)
  elseif ffi.istype('RECT', pt) then
    pt = ffi.cast('LPPOINT', ffi.cast('void *', pt))
    return ClientToScreen(hwnd, pt) and ClientToScreen(hwnd, pt+1)
  --elseif type(pt) == 'table' then
  else
    error('Expected point, not ' .. tostring(pt))
  end
end

function ScreenToClient(hwnd, pt)
  if ffi.istype('POINT', pt) then
    return C.ScreenToClient(hwnd, pt)
  elseif ffi.istype('RECT', pt) then
    pt = ffi.cast('LPPOINT', ffi.cast('void *', pt))
    return ScreenToClient(hwnd, pt) and ScreenToClient(hwnd, pt+1)
  --elseif type(pt) == 'table' then
  else
    error('Expected point, not ' .. tostring(pt))
  end
...
do
  local enumFuncs = {}
  local cb = ffi.cast('WNDENUMPROC', function(hwnd, lparam)
    return enumFuncs[tonumber(lparam)](hwnd) and 1 or 0
  end)
  local nextLparam = 0
  function EnumWindows(enumFunc)
    local lparam = nextLparam
    nextLparam = nextLparam + 1
    enumFuncs[lparam] = enumFunc
    local result = C.EnumWindows(cb, lparam) ~= 0
    enumFuncs[lparam] = nil
    return result
  end
end
...
function SetWindowLongPtr(hwnd, GWL, long)
  return callnz2(SetWindowLongW, hwnd, flags(GWL), ffi.cast('LONG_PTR', long))
end

function GetWindowLong(hwnd, GWL)
  return tonumber(GetWindowLongW(hwnd, flags(GWL)))
end

function GetWindowLongPtr(hwnd, GWL)
...
function AdjustWindowRect(rect, wstyle, hasmenu, wstylex)
  checknz(C.AdjustWindowRectEx(rect, flags(wstyle), hasmenu, flags(wstylex)))
  return rect
end
...
typedef void __stdcall (*SENDASYNCPROC)(
  HWND hwnd,
  UINT uMsg,
  ULONG_PTR dwData,
  LRESULT lResult
...
function GetMessage(hwnd, WMmin, WMmax, msg)
      msg = types.MSG(msg)
  return checkpoz(C.GetMessageW(msg, hwnd, flags(WMmin), flags(WMmax))), msg
end

function PeekMessage(hwnd, WMmin, WMmax, msg, remove)
      msg = types.MSG(msg)
  return checkpoz(C.PeekMessageW(msg, hwnd, flags(WMmin), flags(WMmax), flags(remove))), msg
end
...
local nextAsyncKey = 0
local sendAsyncMap = setmetatable({}, {__mode='v'})

local function sendAsyncProc(hwnd, WM, data, result)
  local delegate = sendAsyncMap[tonumber(data)]
  sendAsyncMap[data] = nil
  if delegate then
    delegate(hwnd, WM, result)
  end
end

local sendAsyncCB = ffi.cast('SENDASYNCPROC', sendAsyncProc)

function SendMessageCallback(hwnd, WM, wparam, lparam, delegate)
  local asyncKey = nextAsyncKey
  nextAsyncKey = nextAsyncKey + 1
  sendAsyncMap[asyncKey] = delegate
  if wparam == nil then wparam = 0 end
  if type(lparam) == 'nil' then lparam = 0 end
  return C.SendMessageCallbackW(hwnd, WM,
        ffi.cast('WPARAM', wparam),
        ffi.cast('LPARAM', lparam),
        sendAsyncCB,
        asyncKey)
...
jit.off(EnumChildWindows)
jit.off(EnumWindows)
jit.off(GetMessage)
jit.off(IsDialogMessage)
jit.off(PeekMessage)
jit.off(SendMessage)
jit.off(SendMessageCallback)
